...
class Corpus_lstm(object):
	def __init__(self, vocab_size, batch_size, embed_size, hidden_size, sequence_length, start_token, params):
...
# Initialize parameters ------------------
		...
		target_prob = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length, dynamic_size=False, infer_shape=True)
		target_token_sequence = tensor_array_ops.TensorArray(dtype=tf.int32, size=self.sequence_length, dynamic_size=False, infer_shape=True)

# End initialize ------------------

# Forward step -------------------

		def _target_recurrence(i, x_t, h_tm, lstm_o, lstm_x):
			...
			lstm_o = lstm_o.write(i, tf.reduce_sum(tf.multiply(tf.one_hot(next_token, self.vocab_size, 1.0, 0.0), o_t), 1))
...
# Pre-train step -------------------------
		...
		target_predictions = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length, dynamic_size=False, infer_shape=True)
		real_x = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length)
		real_x = real_x.unstack(self.processed_x)       # Gain real data's token embedding

		def _pretrain_recurrence(i, x_t, h_tm, o_predictions):
			h_t = self.target_lstm_forward(x_t, h_tm)
			o_t = self.target_linear_forward(h_t)
			o_predictions = o_predictions.write(i, o_t)
			x_ = real_x.read(i)
			return i + 1, x_, h_t, o_predictions
...
# Adversarial Loss configuration ------------------
		...
		self.target_loss = tf.reduce_sum(
			tf.reshape(
				...
				), [-1, self.sequence_length]       # shape = [batch_size, seq_length]
